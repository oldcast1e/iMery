...
import { Camera, Image as ImageIcon, X, ChevronDown, Check } from 'lucide-react-native';
...
export default function UploadScreen() {
    ...
    const [step, setStep] = useState(1); // 1: Image Select, 2: Form
    ...
    const predefinedTags = ['유화', '수채화', '풍경', '인물', '추상', '모던', '고전']; // Simple set for MVP
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            data.append('work_date', formData.date.replace(/-/g, '.')); // Format YYYY.MM.DD
            data.append('genre', formData.genre);
            data.append('style', formData.style);
            data.append('rating', String(formData.rating));
            // Tags might need JSON stringify dependent on backend - assume comma separated or JSON
            // Web code uses array but API likely expects JSON or handles it?
            // Let's verify README_API.md. It doesn't detail tags field. 
            // Check ReviewForm.jsx again? It just saves to state.
            // Assuming backend handles standard fields. If tags are separate table, logic might differ.
            // We'll append JSON string for now or skipping if backend isn't explicit.
                    ...
                    <View className="space-y-6 mb-12">
                        ...
                        <View>
                            ...
                            <View className="flex-row flex-wrap gap-2">
                                ...
                                {/* Add custom tag button could go here */}
                            </View>
                        </View>
