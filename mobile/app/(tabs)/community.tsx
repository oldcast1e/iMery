...
import { Bookmark, Heart, Music, MessageCircle } from 'lucide-react-native';
...
export default function CommunityScreen() {
    ...
    const loadData = async () => {
        try {
            ...
            if (user) {
                // Fetch all posts (or friends posts if API supported it)
                // Currently API returns ALL posts. We need to filter for friends.
                // But friend list fetching + filtering might be heavy.
                // For v2.0 MVP, we will display ALL posts in Community (like a public feed) 
                // OR implement client-side filtering if getFriends is reliable.
                // Let's try to get friends first used in v1.5

                const allPosts = await api.getPosts();

                // Ensure we don't show own posts in community? Or show all?
                // Usually community implies others.
                const otherPosts = allPosts.filter(p => String(p.user_id) !== String(user.user_id || user.id));
