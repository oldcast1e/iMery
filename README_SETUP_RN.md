...
| 버전     | 플랫폼                  | 기술 스택                        | 설명                                                                                                                                                         |
| -------- | ----------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **v2.1** | 📱 Mobile (Update)      | **New Features & Stabilization** | **Feature**: Feed/Archive 탭 분리, Visibility Control<br>**UI**: Facebook-style Feed, Friend Search<br>**Fix**: Upload 500 Error, Tag Encoding, Review Reset |
...
- ✅ **AI 작품 분석**:
  - RunPod (Stable Diffusion/LLM) 연동
  - 이미지 스타일/장르 분석 및 요약 텍스트 생성 (`ai_summary`)
  - **New**: 장르가 "그림"인 경우 스타일 분석 차트 제공, 그 외 장르는 텍스트 요약만 제공
  - **New**: 기존 분석 데이터(`ai_summary`) 우선 사용 규칙 적용 (재분석 방지)
  - 분석 중 로딩 애니메이션 및 완료 토스트 알림 최적화
- **게시글 관리(CRUD)**: 작성, 조회, 수정(데이터 보존 로직 적용), 삭제
- **소셜 기능**: 좋아요, 북마크(폴더링), 댓글
- **캘린더**: 월별 작품 아카이빙 시각화
- **검색 & 태그**: JSON 태그 파싱 자동화 및 검색 기능
- **음악 재생**: 작품 분위기에 맞는 배경음악 자동 재생 및 페이지 이탈 시 자동 중단일
